...
def swine_align(player_score, opponent_score):
    ...
    for num in range(1, min_score + 1):
        if player_score%num==0 and opponent_score%num==0:
            GCD=num
    if GCD>=10:
        return True
    else:
...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    if(opponent_score-player_score>0 and opponent_score-player_score<=2):
        return True
    else:
        return False
    # END PROBLEM 4b
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if score_change > running_high:
            print(f"{score_change} point(s)! The most yet for Player {who}")
            running_high = score_change
