...
def is_bst(t):
    ...
    def bst_max(t):
        if t.is_leaf():
            return t.label  # 如果 t 是叶子节点，则返回其值
        if len(t.branches) == 2:
        # 如果有两个子树，右子树为第二个子节点，递归查找右子树的最大值
            return bst_max(t.branches[1])
        right = t.branches[0]  # 如果只有一个子树，假定为右子树
    # 如果右子树的值大于当前节点，则递归查找右子树的最大值；否则返回当前节点的值
        return bst_max(right) if t.label < right.label else t.label

    def is_bst_helper(t):
        if t.is_leaf() or len(t.branches) == 1:
            return True  # 如果是叶子节点或只有一个子节点，则认为符合 BST 性质
        if len(t.branches) > 2:
            return False  # 如果有超过两个子节点，则不符合 BST 性质
        left, right = t.branches[0], t.branches[1]
    # 递归检查左右子树，并验证左子树的最大值 <= 当前节点值 < 右子树的最小值
